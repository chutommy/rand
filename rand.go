...
// by a function from the crypto/rand package. But since
...
// with the functions which are provided in the math/rand package,
// the generator of pseudo-random numbers.
//
// To achieve the true randomness, the package uses the Read function
// from the crypto/rand package, which by default uses a system's getrandom(2)
// or /dev/urandom functions on Linux/FreeBSD, getentropy(2) on OpenBSD
// and /dev/urandom on other Unix-systems. On Windows systems it uses
...
// to math's rand.Rand, but only without non-functional methods such as Seed()
type Rand struct {
	Mrand *mrand.Rand
}
